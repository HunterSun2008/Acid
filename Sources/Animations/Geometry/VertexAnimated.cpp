...
namespace acid
{
	VertexAnimated::VertexAnimated(const Vector3 &position, const Vector2f &uv, const Vector3 &normal, const Vector3 &tangent, const Vector3 &jointId, const Vector3 &vertexWeight) :
		m_position(position),
		m_uv(uv),
		m_normal(normal),
		m_tangent(tangent),
